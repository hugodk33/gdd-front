...
import { BiArrowBack, BiSolidUserAccount } from 'react-icons/bi';
        ...
        try {
            const response = await axios.get(`http://back.ongprograma.org/clients/search`, {
                ...
                .then(function (response) {
                    setDataForm(response.data.client.data[0])
                })
                    ...
                    <div id="container-principal" className='flex pt-5 w-full'>
                        ...
                        <Content>
                            <div className='w-full flex mb-3'>
                                <div className='w-full flex items-center' style={{ maxWidth: '340px' }}>
                                    <BtnRS href={`/beneficiado/${dataForm?.client_id}`}>
                                        <BiArrowBack style={{ fontSize: '24px', marginTop: '8px' }} />
                                    </BtnRS>
                                    retornar a visualização do beneficiado
                                </div>
                            </div>
                            ...
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pr-1 " >
                                <InputSelect id={'maritial_status'} label="Estado Civil" value={dataForm.maritial_status} onChange={(e) => handleFormSelect(e, 'maritial_status')} options={maritial_status_options} />
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pl-1 ">
                                <InputSelect id={'genre'} label="Sexo" value={dataForm.genre} onChange={(e) => handleFormSelect(e, 'genre')} options={genre_options} />
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pr-1 pt-4" >
                                <InputCalendarForm
                                    id={'birthday'}
                            ...
                            <br /><br />
                            <span className="inline-block w-full sm:w-full md:w-full" >
                                <InputTextArea id={'family'} label="quantos filhos? Sexo e idades" onChange={(e) => handleForm(e)}  >
                                    {dataForm.family}
                                </InputTextArea>
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pr-1 " >
                                <InputSelect id={'welfare_state'} label="Recebe algum auxílio?" value={dataForm.welfare_state} onChange={(e) => handleFormSelect(e, 'welfare_state')} options={welfarestate_status_option} />
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pl-1 " >
                                <InputTextForms id={'welfare_state_type'} label="Quais auxílios?" value={dataForm.welfare_state_type} onChange={(e) => handleForm(e)} />
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pr-1 pt-4" >
                                <InputSelect id={'already'} label="Já foi atendido antes?" value={dataForm.already} onChange={(e) => handleFormSelect(e, 'already')} options={welfarestate_status_option} />
                            </span>
                            ...
                            <span className="inline-block w-full sm:w-full md:w-full pt-4" >
                                <InputTextArea id={'complement'} label="Complemento" onChange={(e) => handleForm(e)} >
                                    {dataForm.complement}
                                </InputTextArea>
                            </span>
                            <span className="inline-block w-full sm:w-full md:w-6/12 md:pr-1 pt-4" >
                            ...
                            <span className="block mr-auto ml-auto sm:w-full md:w-6/12 md:pr-1 pt-4" >
                                <Btn onClick={() => sendForm()}><AiFillSave className="inline-block" /> Editar </Btn>
                            </span>
                        </Content>
                    </div>
                </MainCtnHorizontal>
